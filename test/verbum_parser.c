...
EXIT_0:
	parser_rewind(p, cp);
	{
		if(!parser_match(p, TokenType_COMMENT)) {
			goto EXIT_1;
			// Error, cleanup
		}
		Token COMMENT_0 = parser_previous(p);
		parser_drop(p);
		grammarRepetition0 *grammarRepetition0_0 = parser_parse_grammarRepetition0(p);

		result = verbum_ast_new_grammarRepetition0_choice1(p->ctx, grammarRepetition0_0, COMMENT_0);
		parser_commit(p);
...
EXIT_1:
	parser_rewind(p, cp);
	{
		...
		result = verbum_ast_new_grammarRepetition0_choice2(p->ctx);
		parser_commit(p);
